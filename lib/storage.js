...
export const patientStorage = {
  ...
  // Merges `changes` onto whatever the current stored record is at write
  // time (inside the same CAS cycle as updateList), rather than onto a
  // possibly-stale record the caller read earlier — so a concurrent change
  // to a field not in `changes` (e.g. another tab's soft-delete) survives.
  update(id, changes) {
    const updatedList = storage.updateList(STORAGE_KEYS.PATIENTS, (patients) =>
      patients.map(p => (p.id === id ? { ...p, ...changes } : p))
    );
