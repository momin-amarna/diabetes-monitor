...
- [x] T070 [P] Add error handling + user-friendly error messages across all API routes (createHandler in lib/api-utils.js wraps every route with a method guard + try/catch returning a clean JSON error)
...
- [x] T074 Final manual test pass through all 7 user stories end-to-end (fresh localStorage, production build: login persists across reload, add measurement + notes + AI insight, add/edit/delete a patient, add weight, edit/delete via history, toggle a setting, logout — all confirmed working)
